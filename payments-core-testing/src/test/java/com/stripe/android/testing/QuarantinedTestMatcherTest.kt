...
class QuarantinedTestMatcherTest {
    ...
    fun `match is true when description matches a quarantined class and method`() {
        val matcher = matcherForMatches(
            Match(className = "com.example.Foo", testCaseName = "bar"),
        )
    ...
    fun `match respects multiple quarantined entries`() {
        val matcher = matcherForMatches(
            Match(className = "com.a.First", testCaseName = "m1"),
            Match(className = "com.b.Second", testCaseName = "m2"),
        )
    ...
    private fun matcherForMatches(vararg matches: Match): QuarantinedTestMatcher {
