...
def generate_changelog_entries
    ...
    normalize_changelog_bump_types!(responses)
...
private def parse_changelog_entry_json(stdout)
    json_text = extract_changelog_entry_json_text(stdout)
    JSON.parse(json_text)
rescue JSON::ParserError => e
    raise "changelog_entry_generator.sh returned invalid JSON: #{e.message}\n#{stdout}"
end

private def extract_changelog_entry_json_text(stdout)
    ...
    begin
        ...
        # Fall through and try to recover JSON embedded in prose.
    end

    json_object_start_indices(trimmed).reverse_each do |start_index|
        candidate = extract_balanced_json_object(trimmed, start_index)
...
private def json_object_start_indices(text)
    text.each_char.with_index.filter_map do |char, index|
        index if char == "{"
    end
end

private def extract_balanced_json_object(text, start_index)
    depth = 0
    in_string = false
    escape = false

    text[start_index..].each_char.with_index do |char, offset|
        if escape
            escape = false
            next
        end
        ...
        when "}"
            depth -= 1
            return text[start_index, offset + 1] if depth.zero?
        end
    end
...
private def validate_bump_types!(bump_type, responses)
    bump_types = responses
        .select { |response| response["include_changelog_entry"] }
        .map { |response| response["bump_type"].downcase }
    return if bump_types.empty?
...
CHANGELOG_MESSAGE_TAG_PATTERN = /\A\[(ADDED|CHANGED|FIXED|REMOVED|DEPRECATED|BREAKING|UPDATED)\]/

private def normalize_changelog_bump_types!(responses)
    responses.each do |response|
        next unless response["include_changelog_entry"]

        response["bump_type"] = infer_bump_type_from_message(response["message"])
    end
end

private def infer_bump_type_from_message(message)
    tag_match = message.match(CHANGELOG_MESSAGE_TAG_PATTERN)
    case tag_match&.[](1)
    when "BREAKING", "REMOVED"
        "major"
    when "ADDED", "CHANGED", "DEPRECATED"
        "minor"
    when "FIXED"
        "patch"
    when "UPDATED"
        if message.match?(/minSdkVersion|compileSdkVersion|targetSdkVersion/i)
            "minor"
        else
            "patch"
        end
