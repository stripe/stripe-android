...
- Do all analysis silently. Never write reasoning, commentary, or explanations in the response.
- Never write text before or after the JSON object.
- Never wrap the JSON in markdown code fences.
- Use straight double quotes (`"`) for all JSON strings, not curly/smart quotes.
- The response must parse with `JSON.parse` with no preprocessing.
...
All changed files are under `docs/`, or are `*.md`, `*.txt`, or similar non-code documentation with zero functional source code changes.

Generated Dokka HTML is not a source of truth for API changes. If the diff only touches `docs/` (including `docs/**/navigation.html` and other generated HTML), set `include_changelog_entry: false` even when the HTML diff shows APIs being added or removed. Those API changes belong to the PR that modified the source code, not the Dokka regeneration PR.
...
→ Set true only if the change would produce different behavior when a consumer calls present().
...
Pattern 7: Internal refactor that retains existing behavior -> not observable
An internal code refactor with no visible performance improvements or public API changes is irrelevant to consumers — the benefit is purely for Stripe developers and not for consumers of the SDK. Do not include it.
...
`bump_type` must be consistent with the message tag you chose in Step 3:

Tag	bump_type
[FIXED]	"patch"
[UPDATED] (dependency-only, no SDK requirement changes)	"patch"
[UPDATED] (minSdkVersion / compileSdkVersion / targetSdkVersion change)	"minor"
[ADDED], [CHANGED], [DEPRECATED]	"minor"
[BREAKING], [REMOVED]	"major"

Additional rules:
- Graduating a preview/opt-in API to GA by removing `@RequiresOptIn` or preview annotations is `[CHANGED]` / `"minor"`, not `[BREAKING]` / `"major"`. Callers can keep using the API; they only need to remove `@OptIn`.
- Use `[BREAKING]` / `"major"` only when upgrading requires consumers to change their code to compile or avoid runtime breakage.
- Do not infer API removals from generated `docs/` HTML diffs alone.
...
STEP 5 — Final output
Return only the JSON object. No other text. Do not wrap it in a code block.

Invalid response (do not do this — prose followed by JSON):
Looking at this diff, the changes are internal only...
{"include_changelog_entry": false, "section": "", "message": "", "bump_type": "patch"}

Valid response (do this):
{"include_changelog_entry":false,"section":"","message":"","bump_type":"patch"}

Example with an included entry:
{"include_changelog_entry":true,"section":"PaymentSheet","message":"[FIXED][2532](https://github.com/stripe/stripe-android/pull/2532) Fixed an issue where FlowController would bypass mandate display when setupFutureUsage was added via configureWithIntentConfiguration() after the user had entered card details","bump_type":"patch"}