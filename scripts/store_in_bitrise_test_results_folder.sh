...
#
# Usage: store_in_bitrise_test_results_folder.sh {unit|instrumentation|browserstack}
#
# test-name = path segments before /build (slashes → hyphens).
set -euo pipefail

usage() {
  echo "Usage: $0 {unit|instrumentation|browserstack}" >&2
  exit 1
}
...
case "$1" in
  ...
  browserstack) find_regex='.*/build/browserstack-results$' ;;
  *) usage ;;
esac
...
while IFS= read -r -d '' match_dir; do
  rel="${match_dir#"$ROOT"/}"
  # Module path below ROOT: strip "/build/<anything>"
  prefix="${rel%%/build/*}"
  test_name="${prefix//\//-}"
  dest_dir="$BITRISE_TEST_RESULT_DIR/$test_name"

  mkdir -p "$dest_dir"
  # Copy contents of the test-type folder (JUnit XML, etc.).
  if [[ -n "$(find "$match_dir" -mindepth 1 -print -quit 2>/dev/null)" ]]; then
    cp -R "$match_dir"/. "$dest_dir/"
  fi

  printf '{"test-name": "%s"}' "$test_name" >"$dest_dir/test-info.json"
