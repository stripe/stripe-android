...
create_pr() {
  ...
  echo "Generating PR body with LLM..." >&2
  local pr_template
  pr_template=$(cat .github/PULL_REQUEST_TEMPLATE.md)
  local body
  body=$(echo "$diff" | llm -s "$LLM_SYSTEM_PROMPT" "Fill in this PR template for the given diff. Keep the markdown headings. Write concise, useful content for each section. For the Testing section, check the appropriate boxes. Remove the Screenshots section if there are no UI changes. Remove the HTML comments.

$pr_template")

  echo "" >&2
  echo "── Generated PR body ──" >&2
  echo "$body" >&2
  echo "────────────────────────" >&2
  echo "" >&2
