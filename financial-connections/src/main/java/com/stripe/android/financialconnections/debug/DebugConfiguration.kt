...
internal class DebugConfiguration @Inject constructor(
    ...
    internal val forceNotlinkConsumer: Boolean
        get() = runCatching {
            sharedPreferences.getString("json", null)?.let {
                val jsonObject = Json.decodeFromString(JsonObject.serializer(), it)
                jsonObject[KEY_FORCE_NOTLINK_CONSUMER]?.jsonPrimitive?.contentOrNull == "true"
            } ?: false
        }.getOrDefault(false)
}
...
private const val KEY_FORCE_NOTLINK_CONSUMER = "force_notlink_consumer"
