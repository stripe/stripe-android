...
internal class PassiveChallengeConfirmationDefinition @Inject constructor(
        ...
        return when (result) {
            is PassiveChallengeActivityResult.Failed -> {
                ConfirmationDefinition.Result.NextStep(
                    confirmationOption = confirmationOption.attachHCapctaToken(null),
                    parameters = confirmationParameters
                )
            }
            is PassiveChallengeActivityResult.Success -> {
                ConfirmationDefinition.Result.NextStep(
                    confirmationOption = confirmationOption.attachHCapctaToken(result.token),
                    parameters = confirmationParameters
                )
            }
        }
    ...
    private fun PaymentMethodConfirmationOption.attachHCapctaToken(
        token: String?
    ): PaymentMethodConfirmationOption {
        return when (this) {
