...
internal class SheetActivityResultHandler @Inject constructor(
    private val configuration: EmbeddedActivityArgs.ActivityConfiguration,
    ...
    val shouldReportDismiss: Boolean
        get() = configuration == EmbeddedActivityArgs.ActivityConfiguration.Embedded
    ...
    fun createResult(
        ...
        return when (configuration) {
            EmbeddedActivityArgs.ActivityConfiguration.Embedded -> ActivityResult(
                Activity.RESULT_OK,
                EmbeddedActivityResult.toIntent(originalIntent, result),
            )
            is EmbeddedActivityArgs.ActivityConfiguration.PaymentSheet -> ActivityResult(
                Activity.RESULT_OK,
            ...
            is EmbeddedActivityArgs.ActivityConfiguration.PaymentOptions ->
                result.asPaymentOptionsResult(configuration).let {
    ...
    private fun EmbeddedActivityResult.asPaymentOptionsResult(
        configuration: EmbeddedActivityArgs.ActivityConfiguration.PaymentOptions,
        ...
        return when (this) {
            is EmbeddedActivityResult.Complete ->
                selection
                    ?.withLinkDetails(configuration.initialSelection)
                    ?.let { selection ->
                    ...
                    ?: cancelledPaymentOptionsResult(configuration, paymentMethods)
            is EmbeddedActivityResult.Cancelled,
            is EmbeddedActivityResult.Error -> cancelledPaymentOptionsResult(configuration, paymentMethods)
        }
    }
    ...
    private fun cancelledPaymentOptionsResult(
        ...
        val initialSelection = configuration.initialSelection
            ?.withLinkDetails(configuration.initialSelection)
