...
internal class CheckoutSessionResponseJsonParser(
    ...
    override fun parse(json: JSONObject): CheckoutSessionResponse? {
        ...
        val amount = extractDueAmount(json)
        ...
        // Amount is required for payment mode but optional for setup mode
        if (amount == null && mode != CheckoutSessionResponse.Mode.SETUP) return null
