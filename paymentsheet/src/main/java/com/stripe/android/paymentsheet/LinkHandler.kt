...
internal class LinkHandler @Inject constructor(
    ...
    var linkInlineSelection = MutableLiveData<PaymentSelection.New.LinkInline?>(null)
    ...
    @VisibleForTesting
    val _isLinkEnabled = MutableLiveData<Boolean>()
    val isLinkEnabled: LiveData<Boolean> = _isLinkEnabled

    private val _activeLinkSession = MutableLiveData(false)
    val activeLinkSession: LiveData<Boolean> = _activeLinkSession

    private val _linkConfiguration =
        savedStateHandle.getLiveData<LinkPaymentLauncher.Configuration>(LINK_CONFIGURATION)
    val linkConfiguration: LiveData<LinkPaymentLauncher.Configuration> = _linkConfiguration
