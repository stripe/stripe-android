...
internal class DefaultPaymentMethodVerticalLayoutInteractor(
        ...
        return walletsState?.wallets(WalletLocation.INLINE)
            ?.map { wallet ->
                when (wallet) {
                    ...
                    is WalletsState.SamsungPay -> createSamsungPayDisplayablePaymentMethod()
                }
            } ?: emptyList()
    }
    ...
    private fun PaymentSelection.asVerticalSelection(): PaymentMethodVerticalLayoutInteractor.Selection = when (this) {
        ...
        is PaymentSelection.SamsungPay -> PaymentMethodVerticalLayoutInteractor.Selection.New("samsung_pay")
