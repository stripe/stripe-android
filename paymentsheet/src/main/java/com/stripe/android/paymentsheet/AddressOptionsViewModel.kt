...
internal class AddressOptionsViewModel @Inject internal constructor(
    ...
    val addressResult: MutableStateFlow<PaymentSheet.Address?> = MutableStateFlow(null)
    val formController = formControllerProvider.get()
        .formSpec(LayoutSpec.create(AddressSpec()))
    ...
    fun logSomething() {
        args.config?.merchantDisplayName?.let { Log.d("SKYLER", it) }
        formController.javaClass
    }
    ...
    fun finish() {
        viewModelScope.launch {
            addressResult.emit(PaymentSheet.Address(city = "Seattle"))
        }
    }
