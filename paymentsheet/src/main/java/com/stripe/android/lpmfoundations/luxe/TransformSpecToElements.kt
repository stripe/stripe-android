...
import com.stripe.android.paymentsheet.forms.PlaceholderHelper.specsFromServer
...
internal class TransformSpecToElements(
    ...
    fun transform(
        ...
        applyBillingDetailsConfiguration: Boolean = true,
    ): List<FormElement> {
        val resolvedSpecs = if (applyBillingDetailsConfiguration) {
            ...
            specsFromServer(specs)
        }
        return resolvedSpecs.flatMap { spec ->
            when (spec) {
                ...
                is BoletoTaxIdSpec -> listOf(spec.transform(arguments.initialValues))
                ...
                is KonbiniConfirmationNumberSpec -> listOf(spec.transform(arguments.initialValues))
