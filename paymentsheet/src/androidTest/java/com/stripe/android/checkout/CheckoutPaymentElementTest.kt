    ...
    fun testPaymentOptionsBridgesLinkAccountStateThroughCheckout() {
        ...
        runCheckoutPaymentElementTest(
            ...
            testRules.compose.waitUntil(timeoutMillis = 5_000) {
                testRules.compose.onAllNodes(hasContentDescription(showMenuDescription))
                    .fetchSemanticsNodes(atLeastOneRootRequired = false)
                    .isNotEmpty()
            }
            testRules.compose.onNodeWithContentDescription(showMenuDescription).performClick()
            testRules.compose.onNodeWithTag(LOGOUT_MENU_ROW_TAG).performClick()

            verticalModePage.waitUntilVisible()
            testRules.compose.onNodeWithTag(
                "${TEST_TAG_NEW_PAYMENT_METHOD_ROW_BUTTON}_link"
            ).assertDoesNotExist()
            Espresso.pressBack()
