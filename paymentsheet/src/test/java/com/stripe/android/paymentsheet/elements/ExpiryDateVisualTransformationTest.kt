...
internal class ExpiryDateVisualTransformationTest {
    ...
    fun `verify 19 get separated between 1 and 9`() {
        assertThat(transform.filter(AnnotatedString("19")).text.text)
            .isEqualTo("1 / 9")
    }

    @Test
    fun `verify 123 get separated between 2 and 3`() {
        assertThat(transform.filter(AnnotatedString("123")).text.text)
            .isEqualTo("12 / 3")
    }

    @Test
    fun `verify 093 get separated between 9 and 3`() {
        assertThat(transform.filter(AnnotatedString("093")).text.text)
            .isEqualTo("09 / 3")
    }

    @Test
    fun `verify 53 get separated between 5 and 3`() {
        assertThat(transform.filter(AnnotatedString("53")).text.text)
            .isEqualTo("5 / 3")
    }
}
