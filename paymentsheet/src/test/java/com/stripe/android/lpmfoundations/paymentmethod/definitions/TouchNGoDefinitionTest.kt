...
internal class TouchNGoDefinitionTest {
    ...
    fun `form matches web instructions and mandate`(
        ...
        val formElements = TouchNGoDefinition.formElements(metadata)
        ...
        if (hasSetup) {
            ...
            assertThat(mandate.stringResId).isEqualTo(R.string.stripe_touch_n_go_mandate)
    ...
    fun `terms display never hides terms without changing confirmation requirements`() {
        val metadata = PaymentMethodMetadataFactory.create(
            stripeIntent = LpmBillingAddressTestConfiguration.IntentScenario.SetupIntent
                .stripeIntent(PaymentMethod.Type.TouchNGo),
            termsDisplay = mapOf(PaymentMethod.Type.TouchNGo to PaymentSheet.TermsDisplay.NEVER),
        )
        val formElements = TouchNGoDefinition.formElements(metadata)

        assertThat(formElements.single()).isInstanceOf(StaticTextElement::class.java)
        assertThat(TouchNGoDefinition.requiresMandate(metadata)).isEqualTo(true)
    }
