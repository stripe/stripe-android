...
internal class NgWalletDefinitionTest {
    @Test
    fun `supports the documented intent modes`(
        ...
        assertThat(NgWalletDefinition.isSupported(metadata)).isEqualTo(
            intentScenario == LpmBillingAddressTestConfiguration.IntentScenario.PaymentIntent
        )
        assertThat(NgWalletDefinition.requiresMandate(metadata)).isEqualTo(
            false
        )
    }
    ...
    fun `form matches web instructions and mandate`(
        ...
        val formElements = NgWalletDefinition.formElements(metadata)

        val notice = formElements.single() as MandateTextElement
        assertThat(notice.stringResId).isEqualTo(R.string.stripe_ng_wallet_mor_notice)
    ...
    fun `terms display never hides terms without changing confirmation requirements`() {
        val metadata = PaymentMethodMetadataFactory.create(
            stripeIntent = LpmBillingAddressTestConfiguration.IntentScenario.PaymentIntent
                .stripeIntent(PaymentMethod.Type.NgWallet),
            termsDisplay = mapOf(PaymentMethod.Type.NgWallet to PaymentSheet.TermsDisplay.NEVER),
        )
        val formElements = NgWalletDefinition.formElements(metadata)

        assertThat(formElements).isEmpty()
        assertThat(NgWalletDefinition.requiresMandate(metadata)).isEqualTo(false)
    }

    @Test
    fun `collects configured contact information`() {
        val formElements = NgWalletDefinition.formElements(
            PaymentMethodMetadataFactory.create(
                stripeIntent = PaymentIntentFactory.create(paymentMethodTypes = listOf("ng_wallet")),
    ...
    fun `does not redisplay saved payment methods`() {
        assertThat(NgWalletDefinition.supportedAsSavedPaymentMethod).isFalse()
    }
