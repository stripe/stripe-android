...
internal class TrueMoneyDefinitionTest {
    @Test
    fun `supports the documented intent modes`(
        ...
        val metadata = PaymentMethodMetadataFactory.create(
            stripeIntent = intentScenario.stripeIntent(PaymentMethod.Type.TrueMoney),
        )

        assertThat(TrueMoneyDefinition.isSupported(metadata)).isEqualTo(
            true
        )
        assertThat(TrueMoneyDefinition.requiresMandate(metadata)).isEqualTo(
    ...
    fun `form matches web instructions and mandate`(
        ...
        val formElements = TrueMoneyDefinition.formElements(metadata)
        ...
        if (hasSetup) {
            ...
            assertThat(mandate.stringResId).isEqualTo(R.string.stripe_truemoney_mandate)
    ...
    fun `terms display never hides terms without changing confirmation requirements`() {
        val metadata = PaymentMethodMetadataFactory.create(
            stripeIntent = LpmBillingAddressTestConfiguration.IntentScenario.SetupIntent
                .stripeIntent(PaymentMethod.Type.TrueMoney),
            termsDisplay = mapOf(PaymentMethod.Type.TrueMoney to PaymentSheet.TermsDisplay.NEVER),
        )
        val formElements = TrueMoneyDefinition.formElements(metadata)

        assertThat(formElements.single()).isInstanceOf(StaticTextElement::class.java)
        assertThat(TrueMoneyDefinition.requiresMandate(metadata)).isEqualTo(true)
    }

    @Test
    fun `collects configured contact information`() {
        val formElements = TrueMoneyDefinition.formElements(
            PaymentMethodMetadataFactory.create(
                stripeIntent = PaymentIntentFactory.create(paymentMethodTypes = listOf("truemoney")),
    ...
    fun `does not redisplay saved payment methods`() {
        assertThat(TrueMoneyDefinition.supportedAsSavedPaymentMethod).isFalse()
    }
}
