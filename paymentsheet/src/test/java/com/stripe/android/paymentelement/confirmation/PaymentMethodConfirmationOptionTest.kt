...
class PaymentMethodConfirmationOptionTest {
    ...
    @Test
    fun `Saved option can be created with attestationToken`() {
        ...
        assertThat(option.attestationToken).isEqualTo(attestationToken)
    }

    @Test
    fun `Saved option attestationToken defaults to null`() {
        val option = PaymentMethodConfirmationOption.Saved(
            paymentMethod = PaymentMethodFixtures.CARD_PAYMENT_METHOD,
            optionsParams = null,
            passiveCaptchaParams = null,
        ...
        assertThat(option.attestationToken).isNull()
    }

    @Test
    fun `Saved option copy preserves attestationToken`() {
        val attestationToken = "test_attestation_token"
        val original = PaymentMethodConfirmationOption.Saved(
            paymentMethod = PaymentMethodFixtures.CARD_PAYMENT_METHOD,
            optionsParams = null,
            passiveCaptchaParams = null,
            attestationToken = attestationToken,
        ...
        val copied = original.copy(hCaptchaToken = "new_hcaptcha_token")

        assertThat(copied.attestationToken).isEqualTo(attestationToken)
    }

    @Test
    fun `Saved option updatedForDeferredIntent preserves attestationToken`() {
        val attestationToken = "test_attestation_token"
        val option = PaymentMethodConfirmationOption.Saved(
            paymentMethod = PaymentMethodFixtures.CARD_PAYMENT_METHOD,
            optionsParams = null,
        ...
        val updated = option.updatedForDeferredIntent(
            intentConfiguration = com.stripe.android.paymentsheet.PaymentSheet.IntentConfiguration(
                mode = com.stripe.android.paymentsheet.PaymentSheet.IntentConfiguration.Mode.Payment(
                    amount = 1000,
                    currency = "usd",
                )
            )
        ...
        assertThat(updated.attestationToken).isEqualTo(attestationToken)
    }
}
