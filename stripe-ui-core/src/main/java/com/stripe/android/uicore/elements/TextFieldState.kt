 ...
 * validation message in a different way.  This interface separates how the state
...
interface TextFieldState {
    ...
    /**
     * Indicate if this is a validation message that should be displayed to the user.
     * This cannot be used to determine if the field is valid or not because
     * there are some cases such as incomplete or blank where the validation message is not
     * displayed, but also not valid.
     */
    fun shouldShowValidationMessage(hasFocus: Boolean, isValidating: Boolean = false): Boolean
    ...
    /**
     * If in a state where isValid() returns false, this function returns the validation message.
     * It is up to calling shouldShowValidationMessage to determine if it should be displayed on screen.
     */
    fun getValidationMessage(): FieldValidationMessage?
