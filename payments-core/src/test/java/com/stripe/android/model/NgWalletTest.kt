...
internal class NgWalletTest {
    @Test
    fun `creates payment method without extra required fields`() {
        assertThat(PaymentMethodCreateParams.createNgWallet().toParamMap())
            .containsExactly("type", "ng_wallet")
    }

    @Test
    fun `preserves supplied billing details and metadata`() {
        val params = PaymentMethodCreateParams.createNgWallet(
    ...
    fun `parses payment method type`() {
        val result = PaymentMethodJsonParser().parse(JSONObject("""{"id":"pm_123","type":"ng_wallet"}"""))

        assertThat(result.type).isEqualTo(PaymentMethod.Type.NgWallet)
    }
}
