...
internal class NgBankTransferTest {
    @Test
    fun `creates payment method without extra required fields`() {
        assertThat(PaymentMethodCreateParams.createNgBankTransfer().toParamMap())
            .containsExactly("type", "ng_bank_transfer")
    }

    @Test
    fun `preserves supplied billing details and metadata`() {
        val params = PaymentMethodCreateParams.createNgBankTransfer(
    ...
    fun `parses payment method type`() {
        val result = PaymentMethodJsonParser().parse(JSONObject("""{"id":"pm_123","type":"ng_bank_transfer"}"""))

        assertThat(result.type).isEqualTo(PaymentMethod.Type.NgBankTransfer)
    }
}
