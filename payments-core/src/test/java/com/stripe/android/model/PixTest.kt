...
internal class PixTest {
    @Test
    fun `creates payment method without extra required fields`() {
        assertThat(PaymentMethodCreateParams.createPix().toParamMap())
            .containsExactly("type", "pix")
    }

    @Test
    fun `preserves supplied billing details and metadata`() {
        val params = PaymentMethodCreateParams.createPix(
    ...
    fun `parses payment method type`() {
        val result = PaymentMethodJsonParser().parse(JSONObject("""{"id":"pm_123","type":"pix"}"""))

        assertThat(result.type).isEqualTo(PaymentMethod.Type.Pix)
    }
}
