...
internal class MonduTest {
    @Test
    fun `creates payment method without extra required fields`() {
        assertThat(PaymentMethodCreateParams.createMondu().toParamMap())
            .containsExactly("type", "mondu")
    }

    @Test
    fun `preserves supplied billing details and metadata`() {
        val params = PaymentMethodCreateParams.createMondu(
    ...
    fun `parses payment method type`() {
        val result = PaymentMethodJsonParser().parse(JSONObject("""{"id":"pm_123","type":"mondu"}"""))

        assertThat(result.type).isEqualTo(PaymentMethod.Type.Mondu)
    }
}
