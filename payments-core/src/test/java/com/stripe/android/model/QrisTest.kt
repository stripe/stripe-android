...
internal class QrisTest {
    @Test
    fun `creates payment method without extra required fields`() {
        assertThat(PaymentMethodCreateParams.createQris().toParamMap())
            .containsExactly("type", "qris")
    }

    @Test
    fun `preserves supplied billing details and metadata`() {
        val params = PaymentMethodCreateParams.createQris(
    ...
    fun `parses payment method type`() {
        val result = PaymentMethodJsonParser().parse(JSONObject("""{"id":"pm_123","type":"qris"}"""))

        assertThat(result.type).isEqualTo(PaymentMethod.Type.Qris)
    }
}
