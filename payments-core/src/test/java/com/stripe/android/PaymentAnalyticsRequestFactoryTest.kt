...
class PaymentAnalyticsRequestFactoryTest {
    ...
    fun getPaymentMethodCreationParams() {
        assertThat(
            ...
            mapOf(
                ...
                "app_version" to "0",
                "product_usage" to "[CardInputView]",
                "source_type" to "card",
                "is_development" to "true",
                "session_id" to AnalyticsRequestFactory.sessionId.toString(),
                "network_type" to "2G",
                "locale" to "en_US",
            )
        )
    ...
    fun getEventLoggingParams_withProductUsage_createsAllFields() {
        ...
        assertEquals(ATTRIBUTION.toList().toString(), params[PaymentAnalyticsRequestFactory.FIELD_PRODUCT_USAGE])
        assertEquals(Token.Type.Card.code, params[PaymentAnalyticsRequestFactory.FIELD_TOKEN_TYPE])
        assertEquals(Build.VERSION.SDK_INT.toString(), params[AnalyticsFields.OS_VERSION])
        assertNotNull(params[AnalyticsFields.OS_RELEASE])
        assertNotNull(params[AnalyticsFields.OS_NAME])
        assertEquals(versionCode.toString(), params[AnalyticsFields.APP_VERSION])
    ...
    fun `product_usage param should include defaultProductUsageTokens and method argument`() {
        ...
        assertThat(productUsage).isEqualTo("[Hello, World]")
    }
    ...
    fun `product_usage param should de-dupe defaultProductUsageTokens and method argument`() {
        ...
        assertThat(analyticsRequest.params["product_usage"]).isEqualTo("[Hello]")
    }
