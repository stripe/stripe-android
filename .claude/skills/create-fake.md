...
```kotlin
@Test
fun `verify payment success reported`() = runTest {
    val fake = FakeEventReporter()
    val paymentMethod = PaymentMethodFixtures.CARD_PAYMENT_METHOD

    // Exercise
    fake.onPaymentSuccess(paymentMethod, PaymentEventSource.PaymentSheet)

    // Verify
    assertThat(fake.paymentSuccessCalls.awaitItem()).isEqualTo(
        FakeEventReporter.PaymentSuccessCall(
            paymentMethod = paymentMethod,
            source = PaymentEventSource.PaymentSheet
        )
    )
