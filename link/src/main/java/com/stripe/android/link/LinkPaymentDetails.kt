 ...
 *
 * @param paymentDetails The [ConsumerPaymentDetails2.PaymentDetails] selected by the user
...
sealed class LinkPaymentDetails(
    open val paymentDetails: ConsumerPaymentDetails2.PaymentDetails,
    open val paymentMethodCreateParams: PaymentMethodCreateParams
) : Parcelable {

    /**
     * A [ConsumerPaymentDetails2.PaymentDetails] that is already saved to the consumer's account.
     */
    @Parcelize
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
    class Saved(
        override val paymentDetails: ConsumerPaymentDetails2.PaymentDetails,
        override val paymentMethodCreateParams: PaymentMethodCreateParams
    ) : LinkPaymentDetails(paymentDetails, paymentMethodCreateParams)

    /**
     * A new [ConsumerPaymentDetails2.PaymentDetails], whose data was just collected from the user.
